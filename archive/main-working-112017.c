...
//	Color data0[STRIP_LENGTH] = animation_data[current_frame * STRIP_LENGTH];
//Color data0[STRIP_LENGTH] = {0xFFFFFFFF,0x0,0x0,0xFFFFFFFF};
int framestrip0_ptr = current_frame * STRIP_LENGTH;
int framestrip1_ptr = (((128 + current_frame) % 512) * STRIP_LENGTH);
int framestrip2_ptr = (((256 + current_frame) % 512) * STRIP_LENGTH);
int framestrip3_ptr = (((384 + current_frame) % 512) * STRIP_LENGTH);
	set_strip(0, &animation_data[framestrip0_ptr]);
	set_strip(1, &animation_data[framestrip1_ptr]);
	set_strip(2, &animation_data[framestrip2_ptr]);
	set_strip(3, &animation_data[framestrip3_ptr]);
/*
